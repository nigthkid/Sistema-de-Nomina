...
namespace RHInfosyp
{
    public partial class ReportedeRegalias : Form
    {
        public ReportedeRegalias()
        {
            InitializeComponent();
        }
        ...
        {
            // TODO: esta línea de código carga datos en la tabla 'dBInfosypDataSet7.Bancos' Puede moverla o quitarla según sea necesario.
            this.bancosTableAdapter.Fill(this.dBInfosypDataSet7.Bancos);
        ...
        private void BtnAceptar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    string año = DtpFechaReporteRegalias.Value.Year.ToString();
                    string dia = DtpFechaReporteRegalias.Value.Day.ToString();
                    string mes = DtpFechaReporteRegalias.Value.Month.ToString();

                    if (Convert.ToInt16(dia) < 10)
                    {
                        dia = "0" + DtpFechaReporteRegalias.Value.Day.ToString();
                    }


                    if (Convert.ToInt16(mes) < 10)
                    {
                        mes = "0" + DtpFechaReporteRegalias.Value.Month.ToString();
                    }

                    string fecha = año + "-" + mes + "-" + dia;

                    DtgvReporteRegalias.DataSource = conexdb.SP_ReporteRegalia(CbBancos.Text,fecha);
        ...
        private void BtnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                if (DtgvReporteRegalias.Rows.Count == 0)
                {
                ...
                else
                {
                    ...
                    DtgvReporteRegalias.ClipboardCopyMode = DataGridViewClipboardCopyMode.EnableAlwaysIncludeHeaderText;
                    DtgvReporteRegalias.MultiSelect = true;
                    DtgvReporteRegalias.SelectAll();
                    DataObject dataObj = DtgvReporteRegalias.GetClipboardContent();
                    if (dataObj != null)
                    ...
                    DtgvReporteRegalias.ClearSelection();
                    DtgvReporteRegalias.MultiSelect = false;
